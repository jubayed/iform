...
var __addon__ = {
    ...
    main: (source, tab_spaces, variableName) => {
        ...
        const [type, fieldName, id] = app.getAttributesValue(
            source.attributes,
            ['type', 'name', 'id'],
        );
        ...
        if (['checkbox', 'radio'].includes(type)) {
            ...
            if (options.length) {
                innerHTML = options.map((op, key) => {
                    ...
                    const checked = `{{ op.value === old('${fieldName}', '${op.value}') ? ' checked=""' : ''}}`;
            ...
            if (options.length) {
                innerHTML = options.map(op => {
                    ...
                    const value = op.value ? `value="${op.value}"` : "";
                    const selected = `{{ op.value === old('${fieldName}', '${op.value}') ? ' selected=""'? '' }}`
